...
function simpleCubeMesh(): PositionMesh {
...
export class SceneNode {
    ...
    constructor(device: GPUDevice, modelColorLayout: GPUBindGroupLayout, name?: string) {
        const descModel: GPUBufferDescriptor = {
            size: 4 * 16,
            usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
            mappedAtCreation: false
        };
        if (name) descModel.label = name + ' model';
        this.finalBuf = device.createBuffer(descModel);

        const descColor: GPUBufferDescriptor = {
            size: 4 * 4,
            usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
            mappedAtCreation: false   
        }
        if (name) descColor.label = name + ' color';
