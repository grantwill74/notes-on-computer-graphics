...
export function genCubeSphere(nFaceSubdivs, topo) {
    const verts = [];
    const indis = [];
...
export async function loadCubemapUnfurled(device, url, uvs, faceDim) {
    ...
    for (let face = 0; face < 6; face++) {
        ...
        device.queue.copyExternalImageToTexture({ source: data, origin: [x, y] }, { texture: tex, colorSpace: 'srgb', origin: [0, 0, face] }, [faceW, faceH, 1]);
    }
...
export async function loadCubemap6Images(device, dim, url, flips // list of faces to flip y on
) {
    ...
    for (let face = 0; face < 6; face++) {
        device.queue.copyExternalImageToTexture({ source: datas[face], flipY: flips.includes(face) }, { texture: tex, colorSpace: 'srgb', origin: [0, 0, face] }, [dim, dim, 1]);
    }
    await device.queue.onSubmittedWorkDone();
...
const SPHERE_TOPO = 'triangle-list';
const CUBE_TOPO = 'triangle-strip';
export class Sample14 {
    ...
    sphereSampler;
    cubeSampler;
    constructor(device, context, sphereMap, cubeMap, skyBox) {
        this.device = device;
