...
export function renderSample06(
    device: GPUDevice,
...
): void 
{
    ...
    for (let i = 0; i < 4; i++) {
        ...
        const bg = device.createBindGroup({
            ...
            entries: [
                {
                    binding: 0,
                    resource: matrixBuffers[i] as GPUBuffer,
                },
                {
                    binding: 1,
                    resource: colorBufs[i] as GPUBuffer
                }
            ],
        });
    ...
    const srgbFormat: GPUTextureFormat =
